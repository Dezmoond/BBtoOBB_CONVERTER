...
def get_bbox_from_mask(fatmask):
    contours, hierarchy = cv2.findContours(fatmask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def display_images_with_annotations(image_paths, annotations_list, img_heights, img_widths):
    class IndexTracker:
        def __init__(self, ax, image_paths, annotations_list, img_heights, img_widths):
            ...
            self.fig = ax.figure
            self.ax.imshow(cv2.cvtColor(cv2.imread(image_paths[self.idx]), cv2.COLOR_BGR2RGB))
        ...
        def draw_annotations(self):
            ...
            self.ax.imshow(img)
            for annotation in self.annotations_list[self.idx]:
                _, x1, y1, x2, y2, x3, y3, x4, y4 = annotation
                coords = [x1, y1, x2, y2, x3, y3, x4, y4]
                denormalized_coords = [
                    (x * self.img_widths[self.idx], y * self.img_heights[self.idx])
                    for x, y in zip(coords[0::2], coords[1::2])
                ]
                polygon = patches.Polygon(denormalized_coords, closed=True, edgecolor='green', linewidth=3, fill=False)
                self.ax.add_patch(polygon)
            self.ax.set_title(f"File: {os.path.basename(self.image_paths[self.idx])}")
...
def main(image_folder, annotation_folder, progress_bar, status_label):
    ...
    display_images_with_annotations(image_paths, annotations_list, img_heights, img_widths)
...
def create_gui():
    ...
    # Кнопка запуска
    run_button = tk.Button(root, text="Запуск",
